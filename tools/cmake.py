...
def GenerateCFiles(project):
    ...
    if 'CXX' in dir(fmconfig):
        ...
        CXX = ''
    AS = os.path.join(fmconfig.EXEC_PATH, fmconfig.AS).replace('\\', "/")
    AR = os.path.join(fmconfig.EXEC_PATH, fmconfig.AR).replace('\\', "/")
    LINK = os.path.join(fmconfig.EXEC_PATH, fmconfig.LINK).replace('\\', "/")
    if fmconfig.PLATFORM in ['gcc']:
        SIZE = os.path.join(fmconfig.EXEC_PATH, fmconfig.SIZE).replace('\\', "/")
        OBJDUMP = os.path.join(fmconfig.EXEC_PATH, fmconfig.OBJDUMP).replace('\\', "/")
        OBJCOPY = os.path.join(fmconfig.EXEC_PATH, fmconfig.OBJCPY).replace('\\', "/")
    elif fmconfig.PLATFORM in ['armcc', 'armclang']:
        FROMELF = os.path.join(fmconfig.EXEC_PATH, 'fromelf').replace('\\', "/")

    CFLAGS = fmconfig.CFLAGS.replace('\\', "/").replace('\"', "\\\"")
    if 'CXXFLAGS' in dir(fmconfig):
        CXXFLAGS = fmconfig.CXXFLAGS.replace('\\', "/").replace('\"', "\\\"")
    else:
        CXXFLAGS = CFLAGS
    ...
    if cm_file:
        ...
        cm_file.write("SET(CMAKE_SYSTEM_PROCESSOR " + fmconfig.CPU +")\n")
