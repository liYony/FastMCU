...
def create_need_files(targetId, packpath):
    ...
    if vsc_launch_file:
        ...
        if os.getenv('FM_EXEC_PATH'):
            config_obj['armToolchainPath'] = os.getenv('FM_EXEC_PATH').replace('\\', '/')
        else:
            print('env <FM_EXEC_PATH> not set!')
...
def get_socName_from_fmconfig():
    socName = None
    fmconfig_file = open('fmconfig.h', 'r')
    if fmconfig_file:
        for line in fmconfig_file.readlines():
...
def get_pack_from_env():
    ...
    # get soc name from <fmconfig.h> file
    socName = get_socName_from_fmconfig()
    if socName == None:
        return
