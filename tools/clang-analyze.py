...
def generate(env):
    assert(fmconfig.CROSS_TOOL == 'clang-analyze')
    ...
    if fmconfig.EXEC_PATH:
        if not os.path.exists(fmconfig.EXEC_PATH):
            print()
            print('warning: fmconfig.EXEC_PATH(%s) does not exists.' % fmconfig.EXEC_PATH)
            print()
            return
        env.AppendENVPath('PATH', fmconfig.EXEC_PATH)
