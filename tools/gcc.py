...
def GetGCCRoot(fmconfig):
    exec_path = fmconfig.EXEC_PATH
    prefix = fmconfig.PREFIX
...
def GetGccDefaultSearchDirs(fmconfig):
    start_pattern = r' *#include <\.\.\.> search starts here: *'
    end_pattern = r' *End of search list\. *'

    gcc_cmd = os.path.join(fmconfig.EXEC_PATH, fmconfig.CC)
    device_flags = fmconfig.DEVICE.split()
...
def GetHeader(fmconfig, filename):
    include_dirs = GetGccDefaultSearchDirs(fmconfig)
...
def GetPicoLibcVersion(fmconfig):
    version = None
    try:
        fmconfig.PREFIX
    except:
    ...
    fn = GetHeader(fmconfig, 'picolibc.h')
...
def GetNewLibVersion(fmconfig):
    ...
    fn = GetHeader(fmconfig, '_newlib_version.h')

    # get version from newlib.h
    if not fn:
        fn = GetHeader(fmconfig, 'newlib.h')
...
def GetMuslVersion(fmconfig):
    ...
    if 'musl' in fmconfig.PREFIX:
        version = 'unknown'
    return version

def GCCResult(fmconfig, str):
    result = ''
