...
static fm_err_t stm32_adc_get_channel(fm_int8_t fm_channel, uint32_t *stm32_channel)
{
    switch (fm_channel)
    {
    case  0:
...
static fm_err_t stm32_adc_enabled(struct fm_adc_device *device, fm_int8_t channel, fm_bool_t enabled)
{
    ...
    if (enabled)
    {
        ...
        fm_memset(&ADC_ChanConf, 0, sizeof(ADC_ChanConf));

        if(stm32_adc_get_channel(channel, &ADC_ChanConf.Channel) != FM_EOK)
        {
...
static fm_uint8_t stm32_adc_get_resolution(struct fm_adc_device *device)
{
...
static fm_int16_t stm32_adc_get_vref (struct fm_adc_device *device)
{
    if(device == FM_NULL)
      return -FM_ERROR;
    ...
    fm_err_t ret = FM_EOK;
    fm_uint32_t vref_value;

    ADC_HandleTypeDef *stm32_adc_handler = device->parent.user_data;

    ret = fm_adc_enable(device, FM_ADC_INTERN_CH_VREF);
    if (ret != FM_EOK) return (fm_int16_t)ret;
    vref_value = fm_adc_read(device, FM_ADC_INTERN_CH_VREF);
    ret = fm_adc_disable(device, FM_ADC_INTERN_CH_VREF);
    if (ret != FM_EOK) return (fm_int16_t)ret;
...
static fm_err_t stm32_adc_get_value(struct fm_adc_device *device, fm_int8_t channel, fm_uint32_t *value)
{
    ADC_HandleTypeDef *stm32_adc_handler;

    FM_ASSERT(device != FM_NULL);
    FM_ASSERT(value != FM_NULL);
    ...
    /* get ADC value */
    *value = (fm_uint32_t)HAL_ADC_GetValue(stm32_adc_handler);

    return FM_EOK;
}

static const struct fm_adc_ops stm_adc_ops =
{
...
static int stm32_adc_init(void)
{
    ...
    for (i = 0; i < sizeof(adc_config) / sizeof(adc_config[0]); i++)
    {
        ...
        {
            /* register ADC device */
            if (fm_hw_adc_register(&stm32_adc_obj[i].stm32_adc_device, name_buf, &stm_adc_ops, &stm32_adc_obj[i].ADC_Handler) == FM_EOK)
            {
