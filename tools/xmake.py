...
class XmakeProject:
    ...
    def set_toolchain_path(self):
        self.bindir = os.path.abspath(fmconfig.EXEC_PATH).replace('\\', "/")
        self.sdkdir = self.bindir[:-4]
        # delete -
        self.toolchain = fmconfig.PREFIX[:-1]
    ...
    def set_target_config(self):
        ...
        # 3. config cflags
        self.cflags = fmconfig.CFLAGS.replace('\\', "/").replace('\"', "\\\"")
        # 4. config cxxflags
        if 'CXXFLAGS' in dir(fmconfig):
            self.cxxflags = fmconfig.CXXFLAGS.replace('\\', "/").replace('\"', "\\\"")
        else:
            self.cxxflags = self.cflags
        # 5. config asflags
        self.asflags = fmconfig.AFLAGS.replace('\\', "/").replace('\"', "\\\"")
        # 6. config lflags
        self.ldflags = fmconfig.LFLAGS.replace('\\', "/").replace('\"', "\\\"")
        # 7. config define
    ...
    def generate_xmake_file(self):
        ...
        template_path = os.path.join(FM_ROOT, "tools", "xmake.lua")
