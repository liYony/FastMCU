...
$(eval C_SRC := $(C_SRC:$(FM_ROOT)/%=%)) \
$(eval COBJ := $(1:%.c=%.o)) \
$(eval COBJ := $(COBJ:$(BSP_ROOT)/%=$(BSP_BUILD_DIR)/%)) \
$(eval COBJ := $(COBJ:$(FM_ROOT)/%=$(RTT_BUILD_DIR)/%)) \
...
$(eval CXX_SRC := $(CXX_SRC:$(FM_ROOT)/%=%)) \
$(eval CXXOBJ := $(1:%.cpp=%.o)) \
$(eval CXXOBJ := $(CXXOBJ:$(BSP_ROOT)/%=$(BSP_BUILD_DIR)/%)) \
$(eval CXXOBJ := $(CXXOBJ:$(FM_ROOT)/%=$(RTT_BUILD_DIR)/%)) \
...
$(eval S_SRC := $(S_SRC:$(FM_ROOT)/%=%)) \
$(eval SOBJ := $(1:%.S=%.o)) \
$(eval SOBJ := $(SOBJ:$(BSP_ROOT)/%=$(BSP_BUILD_DIR)/%)) \
$(eval SOBJ := $(SOBJ:$(FM_ROOT)/%=$(RTT_BUILD_DIR)/%)) \
