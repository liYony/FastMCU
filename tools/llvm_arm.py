...
def CheckHeader(fmconfig, filename):
    root = GetLLVM_ARMRoot(fmconfig)
...
def GetPicoLibcVersion(fmconfig):
    version = None
    root = GetLLVM_ARMRoot(fmconfig)
    if CheckHeader(fmconfig, 'picolibc.h'): # get version from picolibc.h file
        for config in os.listdir(root):
