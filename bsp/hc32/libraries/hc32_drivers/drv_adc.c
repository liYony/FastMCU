...
static void _adc_internal_trigger0_set(adc_device *p_adc_dev)
{
    ...
    fm_bool_t is_internal_trig0_enabled = (p_adc_dev->init.hard_trig_src == ADC_HARDTRIG_EVT0 || p_adc_dev->init.hard_trig_src == ADC_HARDTRIG_EVT0_EVT1);

    if (is_internal_trig0_enabled == FM_FALSE)
    {
        return;
    }

#if defined(HC32F4A0)
    switch ((fm_uint32_t)p_adc_dev->instance)
    {
...
{
    uint32_t u32TriggerSel;
    fm_bool_t is_internal_trig1_enabled = (p_adc_dev->init.hard_trig_src == ADC_HARDTRIG_EVT1 || p_adc_dev->init.hard_trig_src == ADC_HARDTRIG_EVT0_EVT1);

    if (is_internal_trig1_enabled == FM_FALSE)
    {
        return;
    }
...
static fm_err_t _adc_enable(struct fm_adc_device *device, fm_int8_t channel, fm_bool_t enabled)
{
    adc_device *p_adc_dev = fm_container_of(device, adc_device, fm_adc);
    ADC_ChCmd(p_adc_dev->instance, ADC_SEQ_A, channel, (en_functional_state_t)enabled);
    return 0;
}

static fm_err_t _adc_convert(struct fm_adc_device *device, fm_int8_t channel, fm_uint32_t *value)
{
    fm_err_t fm_ret = -FM_ERROR;
    ...
    adc_device *p_adc_dev = fm_container_of(device, adc_device, fm_adc);
    if (p_adc_dev->init.hard_trig_enable == FM_FALSE && p_adc_dev->instance->STR == 0)
    {
        ADC_Start(p_adc_dev->instance);
    }

    uint32_t start_time = fm_get_systick();
    do
    {
    ...
    while ((fm_get_systick() - start_time) < p_adc_dev->init.eoc_poll_time_max);
...
static struct fm_adc_ops g_adc_ops =
{
    _adc_enable,
    _adc_convert,
};

static int fm_hw_adc_init(void)
{
    int ret, i = 0;
    ...
    for (; i < dev_cnt; i++)
    {
        ...
        fm_hw_board_adc_init((void *)g_adc_dev_array[i].instance);
        ret = fm_hw_adc_register(&g_adc_dev_array[i].fm_adc, \
...
INIT_DEVICE_EXPORT(fm_hw_adc_init);
#endif
