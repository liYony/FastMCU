...
'''
        ...
        rtt_pre_inc_item = '"${workspace_loc:/${ProjName}/fmconfig_preinc.h}"'
        # save the CPPDEFINES in to fmconfig_preinc.h
        with open('fmconfig_preinc.h', mode = 'w+') as f:
            f.write(file_header)
...
def GenExcluding(env, project):
    rtt_root = os.path.abspath(env['FM_ROOT'])
    bsp_root = os.path.abspath(env['BSP_ROOT'])
