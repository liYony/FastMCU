...
def PrepareBuilding(env, root_directory, has_libcpu=False, remove_components = []):
    ...
    if tgt_name:
        ...
        SetOption('no_exec', 1)
        try:
            fmconfig.CROSS_TOOL, fmconfig.PLATFORM = tgt_dict[tgt_name]
            # replace the 'FM_CC' to 'CROSS_TOOL'
            os.environ['FM_CC'] = fmconfig.CROSS_TOOL
        except KeyError:
    ...
    if exec_prefix:
        os.environ['FM_CC_PREFIX'] = exec_prefix

    # auto change the 'FM_EXEC_PATH' when 'fmconfig.EXEC_PATH' get failed
    if not os.path.exists(fmconfig.EXEC_PATH):
        if 'FM_EXEC_PATH' in os.environ:
            # del the 'FM_EXEC_PATH' and using the 'EXEC_PATH' setting on fmconfig.py
            del os.environ['FM_EXEC_PATH']

    exec_path = GetOption('exec-path')
    if exec_path:
        os.environ['FM_EXEC_PATH'] = exec_path

    utils.ReloadModule(fmconfig) # update environment variables to fmconfig.py

    # some env variables have loaded in Environment() of SConstruct before re-load fmconfig.py;
    # after update fmconfig.py's variables, those env variables need to synchronize
    if exec_prefix:
        env['CC'] = fmconfig.CC
        env['CXX'] = fmconfig.CXX
        env['AS'] = fmconfig.AS
        env['AR'] = fmconfig.AR
        env['LINK'] = fmconfig.LINK
    if exec_path:
    ...
    if GetOption('strict-compiling'):
        STRICT_FLAGS = ''
        if fmconfig.PLATFORM in ['gcc']:
            STRICT_FLAGS += ' -Werror' #-Wextra
            env.Append(CFLAGS=STRICT_FLAGS, CXXFLAGS=STRICT_FLAGS)

    # add compability with Keil MDK 4.6 which changes the directory of armcc.exe
    if fmconfig.PLATFORM in ['armcc', 'armclang']:
        if fmconfig.PLATFORM == 'armcc' and not os.path.isfile(os.path.join(fmconfig.EXEC_PATH, 'armcc.exe')):
            if fmconfig.EXEC_PATH.find('bin40') > 0:
                fmconfig.EXEC_PATH = fmconfig.EXEC_PATH.replace('bin40', 'armcc/bin')
    ...
    if env['PLATFORM'] == 'win32':
        os.environ['PATH'] = fmconfig.EXEC_PATH + ";" + os.environ['PATH']
    else:
        os.environ['PATH'] = fmconfig.EXEC_PATH + ":" + os.environ['PATH']

    # add program path
    env.PrependENVPath('PATH', os.environ['PATH'])
    # add fmconfig.h/BSP path into Kernel group
    ...
    # parse fmconfig.h to get used component
    ...
    if GetOption('clang-analyzer'):
        ...
        fmconfig.POST_ACTION = ''

    # auto append '_REENT_SMALL' when using newlib 'nano.specs' option
    if fmconfig.PLATFORM in ['gcc'] and str(env['LINKFLAGS']).find('nano.specs') != -1:
    ...
    # include drivers
    objs.extend(SConscript(Rtt_Root + '/drivers/SConscript',
                           variant_dir=kernel_vdir + '/drivers',
                           duplicate=0))
...
def PrepareModuleBuilding(env, root_directory, bsp_directory):
    ...
    # parse bsp fmconfig.h to get used component
    PreProcessor = PatchedPreProcessor()
    f = open(bsp_directory + '/fmconfig.h', 'r')
    contents = f.read()
    f.close()
    PreProcessor.process_contents(contents)
    BuildOptions = PreProcessor.cpp_namespace
...
def Preprocessing(input, suffix, output = None, CPPPATH = None):
    if hasattr(fmconfig, "CPP") and hasattr(fmconfig, "CPPFLAGS"):
        if output == None:
            import re
        ...
        for cpppath_item in cpppath:
            inc += ' -I' + cpppath_item
        CPP = fmconfig.EXEC_PATH + '/' + fmconfig.CPP
        if not os.path.exists(CPP):
            CPP = fmconfig.CPP
        CPP += fmconfig.CPPFLAGS
        path = GetCurrentDir() + '/'
        os.system(CPP + inc + ' ' + path + input + ' -o ' + path + output)
    else:
        print('CPP tool or CPPFLAGS is undefined in fmconfig!')
...
def GroupLibName(name, env):
    ...
    if fmconfig.PLATFORM in ['armcc']:
        return name + '_rvds'
    elif fmconfig.PLATFORM in ['gcc']:
        return name + '_gcc'
...
def EndBuilding(target, program = None):
    ...
    if hasattr(fmconfig, 'BSP_LIBRARY_TYPE'):
        Env['bsp_lib_type'] = fmconfig.BSP_LIBRARY_TYPE

    if hasattr(fmconfig, 'dist_handle'):
        Env['dist_handle'] = fmconfig.dist_handle

    Env.AddPostAction(target, fmconfig.POST_ACTION)
    ...
    if not GetOption('help') and not GetOption('target'):
        if not os.path.exists(fmconfig.EXEC_PATH):
            print ("Error: the toolchain path (" + fmconfig.EXEC_PATH + ") is not exist, please check 'EXEC_PATH' in path or fmconfig.py.")
            need_exit = True
