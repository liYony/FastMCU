...
def walk_kconfig(FM_ROOT, source_list):
    for parent, dirnames, filenames in os.walk(FM_ROOT):
        if 'bsp' in parent:
            continue
        if '.git' in parent:
            continue
        ...
        for line in data:
            if line.find('FM_ROOT') != -1:
                if line.find('sys.path') != -1:
                    f.write('# set FM_ROOT\n')
                    f.write('if not os.getenv("FM_ROOT"): \n    FM_ROOT="fastmcu"\n\n')
            f.write(line)

def bsp_update_kconfig_testcases(dist_dir):
    # delete testcases in fastmcu/Kconfig
    if not os.path.isfile(os.path.join(dist_dir, 'fastmcu/Kconfig')):
        return

    with open(os.path.join(dist_dir, 'fastmcu/Kconfig'), 'r') as f:
        data = f.readlines()
    with open(os.path.join(dist_dir, 'fastmcu/Kconfig'), 'w') as f:
        for line in data:
            if line.find('examples/utest/testcases/Kconfig') == -1:
                f.write(line)

def bsp_update_kconfig(dist_dir):
    # change FM_ROOT in Kconfig
    ...
    with open(os.path.join(dist_dir, 'Kconfig'), 'w') as f:
        ...
        for line in data:
            ...
            if line.find('default') != -1 and found:
                ...
                line = line[0:position] + 'default "fastmcu"\n'
                found = 0
            f.write(line)
...
def MkDist(program, BSP_ROOT, FM_ROOT, Env, project_name, project_path):
    ...
    fm_dir_path = os.path.join(dist_dir, 'fastmcu')
    ...
    print('=> components')
    do_copy_folder(os.path.join(FM_ROOT, 'components'), os.path.join(fm_dir_path, 'components'))
    ...
    print('=> include')
    do_copy_folder(os.path.join(FM_ROOT, 'include'), os.path.join(fm_dir_path, 'include'))

    # copy drivers directory
    print('=> drivers')
    do_copy_folder(os.path.join(FM_ROOT, 'drivers'), os.path.join(fm_dir_path, 'drivers'))

    # copy src directory
    print('=> src')
    do_copy_folder(os.path.join(FM_ROOT, 'src'), os.path.join(fm_dir_path, 'src'))

    # copy tools directory
    print('=> tools')
    do_copy_folder(os.path.join(FM_ROOT, 'tools'), os.path.join(fm_dir_path, 'tools'), ignore_patterns('*.pyc'))

    do_copy_file(os.path.join(FM_ROOT, 'Kconfig'), os.path.join(fm_dir_path, 'Kconfig'))
    do_copy_file(os.path.join(FM_ROOT, 'README.md'), os.path.join(fm_dir_path, 'README.md'))

    print('Update configuration files...')
    # change FM_ROOT in SConstruct
